...
Los miembros se registran desde el portal b1.church en la pantalla `/mobile/checkin` (enrutada por `B1App/src/app/[sdSlug]/mobile/components/ScreenRouter.tsx` a `screens/CheckinPage.tsx`). Requiere un usuario conectado y camina los mismos cuatro pasos que el quiosco — servicios → hogar → grupos → completar — contra los extremos idénticos, con estado mantenido en `B1App/src/helpers/CheckinHelper.ts`. Las diferencias del quiosco: el hogar viene del propio `householdId` del usuario conectado (sin paso de búsqueda), y no hay impresión de etiquetas — en su lugar la pantalla de finalización muestra el código de seguridad del lote como un QR (`qrcode.react`) con una sugerencia de "muestra esto en una estación de registro". Si el hogar ya está registrado cuando se carga la página, un botón "Mostrar código de registro" vuelve a mostrar el QR desde el `securityCode` de la visita existente. El registro se graba de inmediato en el momento del envío (no hay estado pendiente); el QR solo impulsa la impresión de etiquetas en el quiosco.

**Impresión de etiquetas de teléfono a quiosco** (`B1Checkin/app/scan.tsx`, alcanzado desde el botón "Escanear código" en la pantalla de búsqueda): el quiosco abre un `CameraView` de `expo-camera` (orientado hacia el frente de forma predeterminada, volteable) escaneando códigos QR. Una carga útil escaneada se acepta cuando es un código simple de 4 caracteres en el alfabeto de código de seguridad, así que tanto el QR de B1App como el bloque QR de una etiqueta impresa funcionan. La pantalla entonces sigue la ruta de reimpresión de salida — `GET /attendance/visits/code/{code}` → `GET /membership/people/ids` → `LabelHelper.getAllLabelsFor(visits, people, code)` → `PrintUI` — y regresa a la búsqueda. No ocurre ninguna escritura de asistencia en el momento del escaneo; solo etiquetas. Los códigos sin visitas activas, las estaciones sin impresora, y los grupos sin etiqueta cada uno muestran una notificación y regresan a la búsqueda.

Los tipos y `ApiHelper`/`ArrayHelper` vienen de `@churchapps/helpers` y `@churchapps/apphelper`; ningún componente de React se comparte con B1Admin.

...
Ang mga miyembro ay nag-check in mula sa b1.church portal sa `/mobile/checkin` screen (routed ng `B1App/src/app/[sdSlug]/mobile/components/ScreenRouter.tsx` sa `screens/CheckinPage.tsx`). Nangangailangan ito ng naka-log in na user at naglalakad ng parehong apat na hakbang bilang ang kiosk — services → household → groups → complete — laban sa magkaparehong endpoints, na may state na hawak ng `B1App/src/helpers/CheckinHelper.ts`. Ang mga pagkakaiba mula sa kiosk: ang household ay nagmula sa sariling `householdId` ng naka-log in na user (walang search step), at walang label printing — sa halip ay ipinapakita ng completion screen ang security code ng batch bilang isang QR (`qrcode.react`) kasama ang hint na "ipakita ito sa isang check-in station". Kung naka-check in na ang household nang ma-load ang page, isang **Show check-in code** button ang muling nagpapakita ng QR mula sa `securityCode` ng existing visit. Nairerecord agad ang check-in sa oras ng pagsumite (walang pending state); ang QR lang ang nagdi-drive sa label printing sa kiosk.

**Pag-print ng label mula sa telepono papunta sa kiosk** (`B1Checkin/app/scan.tsx`, naaabot mula sa **Scan code** button sa lookup screen): binubuksan ng kiosk ang `expo-camera` `CameraView` (front-facing bilang default, mababaligtad) na nag-scan para sa mga QR code. Tinatanggap ang isang na-scan na payload kapag ito ay hubad na 4-character code sa alphabet ng security code, kaya gumagana kapwa ang QR ng B1App at ang QR block ng isang naka-print na label. Sinusunod pagkatapos ng screen ang check-out reprint path — `GET /attendance/visits/code/{code}` → `GET /membership/people/ids` → `LabelHelper.getAllLabelsFor(visits, people, code)` → `PrintUI` — at bumabalik sa lookup. Walang attendance write na nangyayari sa oras ng pag-scan; labels lamang. Ang mga code na walang aktibong visit, mga station na walang printer, at mga grupong walang label ay bawat isa'y nagpapalabas ng toast at bumabalik sa lookup.

Ang mga types at `ApiHelper`/`ArrayHelper` ay nanggagaling sa `@churchapps/helpers` at `@churchapps/apphelper`; walang React components na ibinabahagi sa B1Admin.

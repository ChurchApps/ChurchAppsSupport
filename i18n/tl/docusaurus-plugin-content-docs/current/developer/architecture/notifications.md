...
Ang **Expansion** ay nagmamaterialize ng fire rows para sa nakaambang horizon (isang rolling multi-day window). Ito ay tumatakbo sa nightly timer, at synchronously kapag na-save ang isang definition kaya't nagpapaputok pa rin ang reminder para sa isang last-minute na event. Ang mga scope definition ay fan out sa pamamagitan ng `loadScopeEntities` ng adapter, na gumagawa ng isang occurrence set bawat kongkretong entity; ang mga entity-level na occurrence ay gumagamit ng key na `definitionId:occurrenceISO:offset`, habang ang mga scoped na occurrence ay nag-namespace ayon sa entity id kaya't hindi sila kailanman nagbabanggaan. Ang pag-upsert ng isang occurrence ay **binubuhay muli** ang dating-cancelled na row — ang cancel-then-re-expand ay ang standard na paraan para i-re-sync ang isang reminder matapos magbago ang kaugnay na entity; ang mga row na `sent`, `failed`, o `processing` na ay hindi na ginagalaw.

...
I-pipe ang mga bagong B1 people, givers, o group members sa isang Mailchimp audience upang ang susunod na welcome series, year-end appeal, o volunteer newsletter ay kumukuha mula sa isang listang palaging up to date. Ang wiring ay nabubuhay nang buo sa Zapier (o Make) — ang B1 ay nag-fire ng event, ang Mailchimp ay nauubos ang subscriber.
...
- (Optional) `people:write` kung plano mo ring isa pang Mailchimp → B1 direksyon
...
2. **Action:** `Mailchimp — Add/Update Subscriber`. I-map ang trigger output:
   ...
   - (Optional) `data.id` → isang Mailchimp merge field kung gusto mong panatilihin ang B1's person id kasama.
...
- **Action** — B1: Find Person (lookup sa pamamagitan ng `personId`) upang makuha ang email
- **Action** — Mailchimp: Add Subscriber to Tag (tag `Gave-2026`)

### Ilagay ang group-specific na welcome series

- **Trigger** — B1: New Group Member, filtered sa pamamagitan ng `data.groupId`
- **Action** — Mailchimp: Add Subscriber to Tag na may pangalan ng group; mag-trigger ng iyong kasalukuyang automation sa off na tag
...
Ang Make's [Mailchimp app](https://www.make.com/en/integrations/mailchimp) ay sumasaklaw sa 44 modules — ang wiring ay pareho, na may B1 *Watch Events* trigger na substitusyon sa Zapier. Tingnan ang [Make overview doc](../make) para sa B1 side.

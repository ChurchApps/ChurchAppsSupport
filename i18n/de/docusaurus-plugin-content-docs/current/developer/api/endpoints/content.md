...
| GET | /public/:churchId | Public | — | Auflisten öffentlicher Seiten (url, 	itle, metaDescription); nur isibility = everyone |
...
Blog-Beiträge sind Metadaten über reguläre Seiten: Jeder pageId eines Beitrags referenziert die Seite, die den Body enthält, und die Beitragszeile fügt 	itle, slug (eindeutig pro Kirche), excerpt, uthorId, photoUrl, publishDate, category und 	ags hinzu. Ein Beitrag wird veröffentlicht, sobald publishDate gesetzt und in der Vergangenheit liegt. Siehe [Website-Builder-Architektur](../../architecture/website-builder#blog-posts-over-pages).
...
Pro-Kirchen-URL-Umleitungen (romPath → 	oPath), begrenzt auf 200 pro Kirche. Pfade werden normalisiert (kleingeschrieben, führender Schrägstrich, kein abschließender Schrägstrich) und romPath ist eindeutig pro Kirche. B1App löst diese bei 404ern auf und gibt einen HTTP 308 aus.
...
| POST | / | JWT | Content.Edit | Erstellen oder aktualisieren Sie Umleitungen. Lehnt romPath = toPath ab und erzwingt die 200-Zeilen-Grenze |

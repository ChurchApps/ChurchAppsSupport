...
Проект использует Yarn (защита блокирует `npm install`):
...
:::tip
Вы можете инициализировать базу данных отдельного модуля командой `npm run initdb -- --module=membership` (или `attendance`, `content`, `giving`, `messaging`, `doing`).
:::
...
| `npm run lint` | Запуск ESLint с автоисправлением (ESLint — единственный форматер) |
...
Если вам нужно разрабатывать общую библиотеку (`@churchapps/helpers` или `@churchapps/apihelper`) одновременно с API, создайте её в рабочем пространстве [Packages](https://github.com/ChurchApps/Packages) и добавьте временный Yarn portal в API:

```bash
# В рабочем пространстве Packages
yarn build
...
Это позволяет тестировать изменения библиотеки на API без публикации в npm. Дополнительную информацию см. в разделе [Общие библиотеки](../shared-libraries/#local-development-against-a-consuming-app) — и никогда не фиксируйте разрешение portal, которое ссылка записывает в `package.json`.

## Связанные статьи

- **[База данных](./database)** — архитектура с одной базой данных для модуля

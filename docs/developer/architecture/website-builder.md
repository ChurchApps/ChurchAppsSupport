...
**Staying on the subject.** The user's prompt is the *subject of the page*, not background about the church. `planPage` classifies the request (`home`, `visit`, `about`, `ministries`, `give`, `contact`, `event`, `topic`), and for `event` and `topic` pages the general-church templates (pastor note, sermons, ministries, groups, community impact, weekly times and countdown, video hero) are not even offered, while `details` (when / where / what to bring) and a date `eventCountdown` are. The writer is told never to guess a time, day of the week, price, room or sign-up method (or to claim something is free or not required), and both judges score on-topic-ness. B1Admin passes `pageType` from the plan into each `writePage` call.

**Full pages from short requests.** Pages have three to six middle sections, generous slot lengths, an intro line on card sections and a five-question FAQ; the writer is told to use the space with warmth, explanation and invitation, none of which needs facts, and the repair pass expands any section that comes back thin. Length is never bought with invention, so the other half of the fix is asking: when the request is under ~500 characters, `planPage` also returns `questions`, the few details the request leaves unanswered (chosen by JEV from a per-page-type bank: time, place, what to bring, sign-up… or worship style, pastor, kids programs…). `AddPageModal` shows them as optional fields; answers are appended to the prompt and the page is re-planned, blank answers reuse the first plan. `churchContext.askedQuestions` stops them being offered twice.

**Visuals.** Beyond the hero, photos appear on the quick-link cards, the invitation section, a three-photo `gallery` template and behind the closing call-to-action. Photos are chosen as Pexels search subjects and emitted as `pexels:<term>` placeholders; B1Admin resolves each through `POST /content/stock/search` (falling back to a built-in image where no `pexelsKey` is configured, as on demo and staging). There are deliberately no portrait subjects and the pastor template carries no photo: a stock stranger must never stand in for a real person. For a church with no pages yet, B1Admin applies `suggestedStyle` to the global styles; existing sites keep their look.

...
| 3 | `POST /content/pages/importTree` | B1Admin saves the **first** candidate to finish and opens the preview (~9s after Save) |
| 4 | `POST /content/pageHistory/restoreSnapshot` | "Try another layout" on the preview swaps in another candidate. Candidates are written lazily — two start immediately, the third only if the user asks — and live in memory for the session only (`B1Admin/src/site/aiPageCandidates.ts`). Each switch is reported to `POST /website/feedback`, which only logs it, so the judges can be checked against what people actually keep |
...
**Staying on the subject.** The user's prompt is the *subject of the page*, not background about the church. `planPage` classifies the request (`home`, `visit`, `about`, `ministries`, `give`, `contact`, `event`, `topic`), and for `event` and `topic` pages the general-church templates (pastor note, sermons, ministries, groups, community impact, weekly times and countdown, video hero) are not even offered, while `details` (when / where / what to bring) and a date `eventCountdown` are. A thin request gets a short page: the number of middle sections is capped by how much the user wrote, because a one-line request cannot fill a long page with true statements. The writer is told never to guess a time, day of the week, price, room or sign-up method, and both judges score on-topic-ness. B1Admin passes `pageType` from the plan into each `writePage` call.

**Visuals.** Photos are chosen as Pexels search subjects and emitted as `pexels:<term>` placeholders; B1Admin resolves each through `POST /content/stock/search` (falling back to a built-in image where no `pexelsKey` is configured, as on demo and staging). There are deliberately no portrait subjects and the pastor template carries no photo: a stock stranger must never stand in for a real person. For a church with no pages yet, B1Admin applies `suggestedStyle` to the global styles; existing sites keep their look.

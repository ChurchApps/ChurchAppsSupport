---
title: "Events"
---

# Events

<div class="article-intro">

The Events section on B1.church lets you browse upcoming church events and register for them directly from the website. When your church enables registration for an event, you can sign up yourself and your family members in just a few steps.

</div>

## Browsing Events

Events appear on the church calendar. When you open an event that has registration enabled, a **Register for this Event** button is displayed alongside the event details.

Events with registration enabled show:

- **Event title and description** -- What the event is about.
- **Date and time** -- When the event takes place.
- **Registration button** -- A blue button that takes you to the registration form.

## Registration Status

Events may show different statuses depending on timing and capacity:

- **Open** -- Registration is available and you can sign up.
- **Not Yet Open** -- The registration window has not started. Check back after the opening date.
- **Closed** -- The registration deadline has passed.
- **Full** -- All available spots have been taken.

:::tip
If an event is full, check back later -- spots may open up if someone cancels their registration.
:::

## What You Need

- A B1.church account (or you can register as a guest)
- Access to your church's B1.church website

## Learn More

- [Registering for Events](./registering) -- Step-by-step guide to signing up for an event
- [Self Check-In](../checkin/self-checkin) -- Check in when you arrive at the event
